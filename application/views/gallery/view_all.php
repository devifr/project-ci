  ...
  var ids=new Array()
   ...
   if(a>0){
     if(confirm("Are you sure want to publish?")){
      $.post("<?php echo base_url(); ?>admin/gallery/publish/yes/",{checkid:ids});
  ...
  var ids=new Array()
   ...
   if(a>0){
     if(confirm("Are you sure want to unpublish?")){
      $.post("<?php echo base_url(); ?>admin/gallery/publish/no/",{checkid:ids});
      setTimeout(function(){ location.reload(); $('.checkid').removeAttr('checked'); }, 1000);
     }
    }else{
    alert('No Data Has Choosen');
  ...
  var ids=new Array()
   ...
   if(a>0){
     if(confirm("Are you sure want to Delete?")){
      $.post("<?php echo base_url(); ?>admin/gallery/delete_data/all/",{checkid:ids});
      setTimeout(function(){ location.reload(); $('.checkid').removeAttr('checked'); }, 1000);
     }
    }else{
    alert('No Data Has Choosen');
 ...
 <div id="wrapper">
  ...
  <div id="navigasi">
    <table width="100%" border="0">
      <tr>
        <td width="70%">&nbsp;</td>
        <td width="6%"><div align="center"><a href="<?php echo base_url('admin/gallery/input/');?>"><img src="<?php echo base_url('asset/images/admin/img/add.png');?>" width="40"></a></div></td>
      ...
      <tr>
        <td>&nbsp;</td>
        <td><div align="center" id="blue">Add Gallery </div></td>
            ...
            <tbody>
              ...
              if($result->num_rows()>0){
               foreach ($result->result() as $key => $gal) {
                $id_encrypt = $this->encrypt->encode($gal->id_gallery);
              ?>
                <tr>
                    <td><input type="checkbox" name="checkid[]" class='checkid' value="<?php echo $id_encrypt; ?>" />
                    </td>
                    <td><?php echo $key+1; ?></td>
                    <td><?php echo $gal->judul_gallery; ?></td>
                    <td>
                      <div align="center">
                      <?php if($gal->active_gallery==0){ ?>
                        <a href="<?php echo base_url().'admin/gallery/publish/yes/'.$id_encrypt; ?>" onclick="return confirm('Do You Want To Active This Images?');"><img src="<?php echo base_url('asset/images/admin/img/delete.png');?>" title="Unpublish" width="16" height="16"></a>
                      <?php }else{ ?>
                        <a href="<?php echo base_url().'admin/gallery/publish/no/'.$id_encrypt ?>" onclick="return confirm('Do You Want To Nonactive This Images');"><img src="<?php echo base_url('asset/images/admin/img/oke.png');?>" title="Publish"></a>                    
                      <?php } ?>
                      </div>
                    </td>
                    ...
                    <td><div align="center"><span id="orange"><a class="fancybox" href="<?php echo base_url('gallery/'.$gal->nama_gallery); ?>" data-fancybox-group="gallery" title="<?php echo $gal->judul_gallery ?>"><img src="<?php echo base_url('gallery/'.$gal->nama_gallery); ?>" class="thumbnail" id="view" idimage="<?php echo $id_encrypt; ?>"/></a></div></td>
                    <td><div align="center"><a href="<?php echo base_url().'admin/gallery/delete_data/'.$id_encrypt; ?>" onclick="return confirm('Do You Want Delete This Row?')">
                      <img src="<?php echo base_url('asset/images/admin/img/remove.png');?>" width="16" height="16" title="Delete"></a>
                        <a href="<?php echo base_url().'admin/gallery/edit/'.$id_encrypt; ?>"><img src="<?php echo base_url('asset/images/admin/img/folderup_16.png');?>" width="16" height="16" title="Edit Data"></a>
                        </div></td>
                </tr>
            <?php
            } 
