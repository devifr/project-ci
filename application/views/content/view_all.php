  ...
  var ids=new Array()
   ...
   if(a>0){
     if(confirm("Are you sure want to publish?")){
      $.post("<?php echo base_url(); ?>admin/article/publish/yes/",{checkid:ids});
  ...
  var ids=new Array()
   ...
   if(a>0){
     if(confirm("Are you sure want to unpublish?")){
      $.post("<?php echo base_url(); ?>admin/article/publish/no/",{checkid:ids});
      setTimeout(function(){ location.reload(); $('.checkid').removeAttr('checked'); }, 1000);
     }
    }else{
    alert('No Data Has Choosen');
  ...
  var ids=new Array()
   ...
   if(a>0){
     if(confirm("Are you sure want to Delete?")){
      $.post("<?php echo base_url(); ?>admin/article/delete_data/all/",{checkid:ids});
      setTimeout(function(){ location.reload(); $('.checkid').removeAttr('checked'); }, 1000);
     }
    }else{
    alert('No Data Has Choosen');
 ...
 <div id="wrapper">
  ...
  <div id="navigasi">
    <table width="100%" border="0">
      <tr>
        <td width="70%">&nbsp;</td>
        <td width="6%"><div align="center"><a href="<?php echo base_url('admin/article/input/');?>"><img src="<?php echo base_url('asset/images/admin/img/add.png');?>" width="40"></a></div></td>
        ...
        <table cellpadding="0" cellspacing="0" border="0" id="table" class="tinytable">
            <thead>
                <tr>
                  ...
                  <th width="5%"><h3>Publish Date</h3></th>
                  <th width="12%"><h3>Kategori</h3></th>
            ...
            <tbody>
              ...
              if($result->num_rows()>0){
               foreach ($result->result() as $key => $menu) {
                $id_encrypt = $this->encrypt->encode($menu->id_content);
              ?>
                <tr>
                    <td><input type="checkbox" name="checkid[]" class='checkid' value="<?php echo $id_encrypt; ?>" />
                    </td>
                    <td><?php echo $key+1; ?></td>
                    <td><?php echo $menu->judul_content; ?></td>
                    <td>
                      <div align="center">
                      <?php if($menu->active_content==0){ ?>
                        <a href="<?php echo base_url().'admin/article/publish/yes/'.$id_encrypt; ?>" onclick="return confirm('Do You Want To Active This Article?');"><img src="<?php echo base_url('asset/images/admin/img/delete.png');?>" title="Unpublish" width="16" height="16"></a>
                      <?php }else{ ?>
                        <a href="<?php echo base_url().'admin/article/publish/no/'.$id_encrypt ?>" onclick="return confirm('Do You Want To Nonactive This Article');"><img src="<?php echo base_url('asset/images/admin/img/oke.png');?>" title="Publish"></a>                    
                      <?php } ?>
                      </div>
                    </td>
                    <td><div align="center"><span id="orange"><?php echo $menu->nama_bahasa; ?></span> </div></td>
                    <td><div align="center"><span id="orange"><?php echo $menu->publish_date; ?></span> </div></td>
                    <td><div><span id="orange"><?php echo $menu->nama_kategori; ?></span></div></td>
                    <td><div align="center"><span id="abu"><?php echo $menu->hits; ?></span></div></td>
                    <td><div align="center"><a href="<?php echo base_url().'admin/article/delete_data/'.$id_encrypt; ?>" onclick="return confirm('Do You Want Delete This Row?')">
